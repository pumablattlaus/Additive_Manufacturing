...
class Get_path(smach.State):
    def __init__(self):
        ...
        launch_ros_node("parse_path_trafo","match_amc","parse_path_trafo.py")


    def execute(self, userdata):
        mir_path_topic = rospy.get_param("~mir_path_topic","/mir_path")
        rospy.loginfo('Witing for path')
        state_publisher("idle")

        global path
        path = rospy.wait_for_message(mir_path_topic, Path)
        rospy.loginfo('formation path received')
        state_publisher("moving_to_start_pose")
        start_pose = path.poses[0].pose
        next_pose = path.poses[10].pose
        ...
        for i in range(0,active_robots):
            ...
            theta = math.atan2(next_pose.position.y - start_pose.position.y, next_pose.position.x - start_pose.position.x)
            theta += math.pi
    ...
    def execute(self, userdata):
        ...
        process = launch_ros_node("formation_controller_node","formation_controller","formation_controller_node.py", 
                                  "", "", path_array=path_array, active_robots=active_robots,
                                    robot_names=robot_names, relative_positions_x=relative_positions_x, relative_positions_y=relative_positions_y,velocity_limit_lin=0.3,
                                    Ky = 1)
